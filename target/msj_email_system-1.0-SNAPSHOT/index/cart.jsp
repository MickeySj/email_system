        ...
        <div id="myCart">
            ...
            <c:forEach items="${cartList}" var="cart">
                <div class="cart-cont">
                    <ul class="ul1">
                        <li class="li2">
                            <a href="/goods/detail?id=${cart.good.id}">
                                <img src="../index/${cart.good.cover}">
                            </a>
                        </li>
                        <li class="li3">
...
<script type="text/javascript">
    ...
    function cartAdd(obj, id) {
        ...
        $.post("/cart/cartAdd", {id: id}, function (data) {
            layer.close(index);
            if (data) {
                $(obj).prev().val(num + 1);
                updateSubtotal(obj); // 更新小计
                reloadTotal(); // 更新订单总额
                updateCartnum($(".cartnum")); //更新购物车
            } else {
                layer.msg("失败了");
            }
        });
    ...
    function cartLess(obj, id) {
        ...
        var index = layer.load();
        $.post("/cart/cartLess", {id: id}, function (data) {
            layer.close(index);
            if (data) {
                if (num == 1) {
    ...
    function cartDelete(obj, id) {
        ...
        var index = layer.load();
        $.post("/cart/cartDelete", {id: id}, function (data) {
            layer.close(index);
            if (data) {
                $(obj).parents(".cart-cont").fadeOut(300);
                reloadTotal(); // 更新订单总额
